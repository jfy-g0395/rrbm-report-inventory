 ...
 *
 * Before the fix: a sales row whose Product Code did not match any product parsed
 * as VALID (with a null productId). It committed as a real order but
 * InventoryService.deductStockForOrder() silently skipped the null-productId item,
 * so revenue was recorded without any stock being deducted. This was especially
 * likely for ECOMMERCE rows, whose marketplace SKUs rarely equal the internal code.
 *
 * After the fix: an unmatched-but-non-blank Product Code is a hard needsFix error,
 * so it can never reach commit unresolved.
 */
@SpringBootTest
...
class ImportProductCodeGapTest {
    ...
    // The reported scenario: an ECOMMERCE row with a marketplace SKU that has no
    // matching internal product. Must land in needsFix, NOT valid.
    @Test
    void ecommerceUnmatchedProductCode_goesToNeedsFix_notValid() throws Exception {
        ...
        mockMvc.perform(multipart("/api/import/upload/sales")
                ...
                // Rejected: the whole receipt has no valid items → not committable
