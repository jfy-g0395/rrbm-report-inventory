...
# PART 3 — Daily Report Backlog (deferred)

Held until Parts 1 and 2 are implemented, because its fields mirror the finished expense and order forms; building it sooner risks rework. When revisited, the intended shape is a mobile-friendly web form with offline storage that syncs when the system is back, plus an Excel template for bulk entry, using temporary receipt numbers (`TEMP-YYYYMMDD-NNNN`) and conflict resolution (skip / update / review) on sync. Full detail is parked in this appendix only.
...
Expense track first (self-contained, nothing customer-facing): E1 schema + role-constraint fix → E2 entry + backdating → E3 void/return → E4 daily report + dashboard widget → E5 monthly report + export. Then agent track (touches live orders): A1 registry → A2 order integration behind a feature flag → A3 commission engine → A4 release workflow + statements → A5 cash-flow widget. Part 3 deferred. (The session-level detail and tests live in `BUILD_PLAN.md`.)
